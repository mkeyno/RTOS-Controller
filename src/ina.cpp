...
extern uint32_t MainWSclinet;
extern bool  client_active ;

#include "param.h"
extern PARAM Param;

#define PressureCAN 20030 
...
uint8_t INA219::begin() {
  ...
  period=200;
  Param.this_reg_a=1.0045;
  Param.this_reg_b=6.8545;
  uint8_t ret;
    Wire.begin();
    ret = reset();
    if (ret)
...
void  INA219::measure(void){ if(millis()-last_measurment>period){
 
//Param.this_current=map( shuntCurrent()*1000,4.0,20.0,0.0,4.0);
Param.this_current= shuntCurrent()*1000;
//if(Param.this_current>19 && ) ledcWrite(0, 128); //CAN.sendImpFormat(20030,666,3,0,Param.this_pressure); errore message 
Param.this_pressure= Param.this_current*Param.this_pre_cal;

//PRINTS("current,",Param.this_current);
Param.information["current"]=Param.this_current;
Param.information["pressure"]=Param.this_pressure;

if(Param.old_pressure!=Param.this_pressure)  
CAN.sendImpFormat(PressureCAN,0,3,0,Param.this_pressure);
 
 Param.old_pressure    =Param.this_pressure;
