...
#define D_SHUNT            0.1
#define D_V_BUS_MAX        16
#define D_V_SHUNT_MAX      0.075
#define D_I_MAX_EXPECTED   20
...
class INA219
{
  public:
    ...
    /// calibration of equations and 
    /// default values are for a 0.25 Ohm shunt on a 5V bus with max current of 1A calibrate(R_SHUNT0.1, V_SHUNT_MAX0.075, V_BUS_MAX16, I_MAX_EXPECTED20);
    ...
    /// Config values (range, gain, bus adc, shunt adc, mode). configure( RANGE_16V,  GAIN_2_80MV,  ADC_16SAMP,  ADC_16SAMP,  CONT_SH_BUS);
    void configure( t_range range   = RANGE_16V,   ///< Range for bus voltage.
    		        ...
    		        t_mode mode     = CONT_SH  ///< Sets operation mode.
    		        );
  ...
  private:
    ...
    unsigned long last_measurment,period;

    ///Read 16 word from given register address.
    int16_t read16( t_reg addr ) const; ///< Register address.
